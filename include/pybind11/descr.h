...
inline constexpr descr<1, 0> _(char c) { return _({ c, '\0' }); }
template <typename Type> constexpr descr<1, 1> _(char c) { return _<Type>({ c, '\0' }); }
...
template <typename Type> constexpr descr<2, 1> _() {
    return _<Type>({ PYBIND11_DESCR_AUTO_TYPE_NAME, '\0' });
}
